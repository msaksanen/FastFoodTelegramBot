...
namespace FastFoodTelegramBot.Commands
{
    class RegInfoCmd : ICommand
    {
        internal static async Task Command(ITelegramBotClient botClient, long id, string? msg, string? firstName, string? lastName)
        {
            ...
            string? info = foundAccount.ToString();
            await botClient.SendTextMessageAsync(id, info, replyMarkup: KbdMaker.InKeyboard(CommandNames.RegBtnCmd));
            return;
